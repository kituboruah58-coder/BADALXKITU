        ...
        try:
            if getattr(sys, "frozen", False):
                ...
                if self.sidecar_config_file.exists():
                    sidecar_api_base = self.sidecar_config_file.read_text(encoding="utf-8").strip()
        except Exception:
            sidecar_api_base = ""
    ...
    def _login_worker(self, username: str, password: str):
        try:
            ...
            if has_placeholder_server_url(host_hint):
                ...
                message = (
                    f"Cannot reach auth server: {host_hint}. "
                    "For client PCs, use your public server URL (not localhost)."
                )
            self.root.after(0, lambda: self._login_error(message))
        except Exception as error:
